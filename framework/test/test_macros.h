...
((real) == (expected)) ? \
++success_counter_g : \
printf(BOLDRED "TEST number: %d: %s, Failed on line %d, Real: %lx, Expected: %lx\n" DEF, test_counter_g, name, __LINE__, (unsigned long) (real), (unsigned long) (expected)); \
} while (0)


#define PASS (test_counter_g == success_counter_g) ? \
printf(BOLDMAGENTA "Passed everything!\n" DEF) : \
